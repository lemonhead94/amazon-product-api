...
class AmazonScraper {
    ...
    grapAsinDetails(body) {
        ...
        output.formats = this.extractOtherFormats($);
    ...
    /**
     * Extract data from section "Formats on Amazon"
     * @param {*} $
     */
    extractOtherFormats($) {
        var asins = [];
        $("#formats").find('a').each(function() {
            var link = $(this).attr('href');
            if(link && link.match(/dp/)){
                asins.push(link.split('dp/').pop().split('/ref')[0]);
            };
        });
        return asins.filter(function(elem, pos) {
            return asins.indexOf(elem) == pos;
        });
    }
